...
async def _make_api_request(method: str, url: str, **kwargs) -> httpx.Response | Dict[str, Any]:
...
async def get_session(session_id: str) -> Dict[str, Any]:
    ...
    res = await _make_api_request("GET", url)
    if isinstance(res, dict):
        return res
    return res.json()
...
async def list_activities(session_id: str, page_size: Optional[int] = 20, page_token: Optional[str] = None) -> Dict[str, Any]:
    ...
    res = await _make_api_request("GET", url, params=params)
    if isinstance(res, dict):
        return res
    return res.json()
...
async def approve_session_plan(session_id: str) -> Dict[str, Any]:
    ...
    res = await _make_api_request("POST", url, json={})
    if isinstance(res, dict):
        return res
...
async def send_session_message(session_id: str, message: str) -> Dict[str, Any]:
    ...
    res = await _make_api_request("POST", url, json=payload)
    if isinstance(res, dict):
        return res
