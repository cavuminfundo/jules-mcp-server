...
async def _make_api_request(method: str, url: str, success_statuses: Tuple[int, ...] = (200,), **kwargs) -> Tuple[Optional[httpx.Response], Optional[Dict[str, str]]]:
    async with httpx.AsyncClient() as client:
        res = await client.request(method, url, headers=get_headers(), timeout=15.0, **kwargs)
        if res.status_code not in success_statuses:
            return None, {"error": f"API error {res.status_code}: {res.text}"}
        return res, None
...
async def list_sessions(page_size: Optional[int] = 10, page_token: Optional[str] = None) -> Dict[str, Any]:
    ...
    res, err = await _make_api_request("GET", f"{JULES_API_BASE}/sessions", params=params)
    if err:
        return err
    return res.json()
...
async def get_session(session_id: str) -> Dict[str, Any]:
    ...
    res, err = await _make_api_request("GET", url)
    if err:
        return err
    return res.json()
...
async def list_activities(session_id: str, page_size: Optional[int] = 20, page_token: Optional[str] = None) -> Dict[str, Any]:
    ...
    res, err = await _make_api_request("GET", url, params=params)
    if err:
        return err
    return res.json()
...
async def approve_session_plan(session_id: str) -> Dict[str, Any]:
    ...
    res, err = await _make_api_request("POST", url, success_statuses=(200, 204), json={})
    if err:
        return err
...
async def send_session_message(session_id: str, message: str) -> Dict[str, Any]:
    ...
    res, err = await _make_api_request("POST", url, success_statuses=(200, 204), json=payload)
    if err:
        return err
